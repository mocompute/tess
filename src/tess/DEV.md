...
2. Synthesize types for every distinct tuple type in the program.
...
3. Synthesize types for every user defined type in the program. These
...
4. Assign type variables to every symbol in the program, where
   ...
   symbols such as labels in a labelled tuple or user type do not hold
   their own types.

5. Without considering function applications, collect constraints from
...
6. Examine each callsite of a named function application, and generate
...
7. Now considering function applications, repeat the constraint

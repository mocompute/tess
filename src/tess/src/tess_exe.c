...
typedef struct {
    allocator        *arena;
    allocator        *compile_arena; // heavy compilation work; parented under budgeted allocator
...
noreturn void usage(int status, char const *argv0) {
    ...
    printf("    -v                     verbose logging (-v phase markers, -vv key decisions, -vvv full "
           "detail)\n");
    printf(
      "    --memory-limit <amt>   specify memory limit for compilation (default 75%% of available mem)\n");
...
void state_deinit(state *self) {
    ...
    if (self->compile_arena) arena_destroy(&self->compile_arena);
    arena_destroy(&self->arena);
    alloc_default_budgeted_allocator_free();

    alloc_invalidate(self);
}

static size_t parse_memory_limit(char const *arg) {
    char              *end;
    unsigned long long val = strtoull(arg, &end, 10);
    if (end == arg) fatal("--memory-limit: invalid number '%s'", arg);
    switch (*end) {
    case 'G':
    ...
    case '\0': break;
    default:   fatal("--memory-limit: unknown suffix '%c' (use K, M, or G)", *end);
    }
    return (size_t)val;
}
    ...
    for (int i = 1; i < argc; ++i) {
        if ('-' == argv[i][0] && argv[i][1] != '\0') {
            ...
            if (0 == strncmp("-o", argv[i], 2)) {
                ...
                break;
            } else if (0 == strcmp("--memory-limit", argv[i])) {
                if (++i >= argc) fatal("--memory-limit requires a value");
                self->memory_limit = parse_memory_limit(argv[i]);
                continue;
...
int compile(state *self) {
    ...
    tl_infer     *infer         = tl_infer_create(self->compile_arena, &infer_opts);
    ...
    parser *parser = parser_create(self->compile_arena, &parser_opts);
    if (!parser) fatal("could not create parser");
    parser_set_verbose(parser, self->verbose_parse);

    allocator     *nodes_alloc = arena_create(self->compile_arena, 64 * 1024);
    ...
    // parser second pass
    parser = parser_create(self->compile_arena, &parser_opts);
    ...
    if (self->report_stats) {
        ...
        self->stats.parse_peak_mem =
          ast_stats.peak_allocated + token_stats.peak_allocated + temp_stats.peak_allocated;
        self->stats.parse_capacity     = ast_stats.capacity + token_stats.capacity + temp_stats.capacity;
        self->stats.parse_final_mem    = ast_stats.allocated + token_stats.allocated + temp_stats.allocated;
    ...
    transpile *transpile = transpile_create(self->compile_arena, &transpile_opts);
...
int main(int argc, char *argv[]) {
    ...
    // Create budgeted allocator from parsed --memory-limit (or default to 75% of available RAM).
    // The options arena uses default_allocator() and stays alive for the program's lifetime.
    // All subsequent arenas (parser, infer, transpile) go through the budget.
    size_t memory_limit = self.memory_limit;
    if (memory_limit == 0) memory_limit = platform_available_memory() / 4 * 3;
    if (memory_limit == 0) {
        fprintf(stderr, "error: unable to determine available memory. Use --memory-limit option.\n");
        exit(1);
    }
    if (self.verbose) fprintf(stderr, "memory limit: %zu\n", memory_limit);
    alloc_default_budgeted_allocator_set(budgeted_create(default_allocator(), memory_limit));
    self.compile_arena = arena_create(get_budgeted_allocator(), 1024 * 1024);

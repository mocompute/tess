...
typedef struct {
    ...
    hashmap         *modules_seen; // str hset of non stdlib modules seen in source code scan

    hashmap         *import_defines;         // hset for conditional compilation during import resolution
    int              conditional_skip_depth; // internal parser value to track ifdefs
...
void state_init(state *self) {
    ...
    self->modules_seen         = hset_create(self->arena, 8);
...
static void process_hash_directive(state *self, str_array *imports, char const *data, u32 capture_start,
                                   u32 pos, int is_stdlib_file) {
...
static void scan_directives(state *self, char_csized input, str_array *imports, int is_stdlib_file) {
    ...
    while (pos < size) {
        ...
        case stop_hash: {
            process_hash_directive(self, imports, data, capture_start, pos, is_stdlib_file);
            state = start;
        } break;
        }
    }
...
static void collect_imports_from_import(state *self, str import_path, str importing_file,
    ...
    scan_directives(self, (char_csized){.size = size, .v = data}, &file_imports,
                    import_resolver_is_stdlib_file(self->resolver, result.canonical_path));
...
static void scan_file_directives(state *self, str path, str_array *resolved_paths) {
    ...
    str_array file_imports = {.alloc = self->arena};
    scan_directives(self, (char_csized){.size = size, .v = data}, &file_imports,
                    import_resolver_is_stdlib_file(self->resolver, path));

...
static int test_subchain_ids(void) {
    ...
    error += tl_monotype_integer_subchain(lookup(reg, "CUnsignedLongLong")) != TL_INTEGER_SUBCHAIN_C_UNSIGNED;
...
static int test_compare_width_same_chain(void) {
    ...
    error += tl_monotype_compare_integer_width(lookup(reg, "CUnsignedChar"), lookup(reg, "CUnsignedLongLong")) != -1;
    error += tl_monotype_compare_integer_width(lookup(reg, "CUnsignedLongLong"), lookup(reg, "CUnsignedChar")) != 1;
...
static int test_same_subchain(void) {
    ...
    error += tl_monotype_same_integer_subchain(lookup(reg, "CUnsignedChar"), lookup(reg, "CUnsignedLong"))   != 1;
